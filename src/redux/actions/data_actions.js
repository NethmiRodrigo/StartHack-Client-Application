...
export const addSong = (formData) => async (dispatch) => {
  dispatch({ type: LOADING });
  try {
    let results = await axoios.post("/add-song", formData);
    dispatch({
      type: CLEAR_ERRORS,
    });
    return results;
...
export const addPlaylist = (playlist) => async (dispatch) => {
  dispatch({ type: LOADING });
  try {
    let results = await axoios.post("/create-playlist", playlist);
    dispatch({
      type: CLEAR_ERRORS,
    });
    return results;
...
export const addSongsToPlaylist = (playlist_id, songs) => async (dispatch) => {
  dispatch({ type: LOADING });
  try {
    let results = await axoios.post(`/playlist-add/${playlist_id}`, { songs });
    dispatch({
      type: CLEAR_ERRORS,
    });
    return results;
...
export const playPlaylist = (roomId, playlistId) => async (dispatch) => {
  try {
    let results = await axoios.post("/play-playlist", { roomId, playlistId });
...
export const changeVolume = (roomId, volume) => async (dispatch) => {
  try {
    let results = await axoios.post("/change-volume", {
      roomId,
